...
const OCCASION_VARIANTS: Array<VariantEntry<OccasionTag>> = [
  ...
  { canonical: 'office', variants: ['office', 'ofis', 'work', 'iş', 'professional'] },
  ...
  { canonical: 'dinner', variants: ['dinner', 'yemek', 'aksam yemegi', 'akşam yemeği', 'date', 'romantik', 'romantic'] },
  { canonical: 'sport', variants: ['sport', 'spor', 'gym', 'training', 'koşu', 'kosu', 'running', 'run'] },
...
const STYLE_VARIANTS: Array<VariantEntry<StyleTag>> = [
  ...
  { canonical: 'sporty', variants: ['sporty', 'spor', 'athleisure', 'koşu', 'kosu', 'running', 'run'] },
...
const SEASON_VARIANTS: Array<VariantEntry<string>> = [
  { canonical: 'spring', variants: ['spring', 'ilkbahar'] },
  { canonical: 'summer', variants: ['summer', 'yaz'] },
  { canonical: 'autumn', variants: ['autumn', 'fall', 'sonbahar'] },
  { canonical: 'winter', variants: ['winter', 'kis', 'kış'] },
  { canonical: 'all season', variants: ['all season', 'dort mevsim', 'dört mevsim'] },
];
...
export function canonicalizeSeason(value?: string) {
  if (!value) return undefined;
  return matchCanonicalValue(value, SEASON_VARIANTS) ?? normalizeSearchText(value);
}
...
export function canonicalizeStyleTags(values: string[]) {
  return dedupeStrings(
    values
      .map((value) => canonicalizeStyleTag(value))
      .filter((value): value is StyleTag => Boolean(value)),
  );
}
...
export function canonicalizeOccasionTags(values: string[]) {
  return dedupeStrings(
    values
      .map((value) => canonicalizeOccasion(value))
      .filter((value): value is OccasionTag => Boolean(value)),
  );
}

export function canonicalizeSeasonTags(values: string[]) {
  return dedupeStrings(values.map((value) => canonicalizeSeason(value)).filter((value): value is string => Boolean(value)));
}
...
export function extractVariantTokens(parts: Array<string | undefined>) {
  ...
  const baseTokens = dedupeStrings(normalizedParts.flatMap((part) => tokenizeSearchText(part)));
  const expanded = new Set(baseTokens);

  [
    ...findEntriesInText(normalizedParts.join(' '), CATEGORY_VARIANTS),
    ...findEntriesInText(normalizedParts.join(' '), OCCASION_VARIANTS),
    ...findEntriesInText(normalizedParts.join(' '), STYLE_VARIANTS),
  ].forEach((canonical) => {
    expanded.add(normalizeSearchText(canonical));
  });

  if (extractCanonicalColor(normalizedParts.join(' '))) {
    expanded.add(normalizeSearchText(extractCanonicalColor(normalizedParts.join(' '))!));
  }
...
export function normalizeProfileSearchInputs(profile: StyleProfile): StyleProfile {
  return {
    ...
    colors: dedupeStrings(profile.colors.map((color) => canonicalizeColor(color) ?? color)),
    styles: canonicalizeStyleTags(profile.styles),
    occasions: dedupeStrings(profile.occasions.map((occasion) => canonicalizeOccasion(occasion) ?? occasion)),
    fitPreference: canonicalizeFit(profile.fitPreference),
  };
}

export function normalizeCandidateSearchMetadata(candidate: ProductSearchCandidate): ProductSearchCandidate {
  return {
    ...candidate,
    color: canonicalizeColor(candidate.color) ?? candidate.color,
    sizes: dedupeStrings(candidate.sizes.map((size) => normalizeSize(size))),
    fit: canonicalizeFit(candidate.fit),
    modesty: canonicalizeModesty(candidate.modesty) ?? candidate.modesty,
    season: canonicalizeSeasonTags(candidate.season),
    styleTags: canonicalizeStyleTags(candidate.styleTags),
    vibeTags: dedupeStrings(candidate.vibeTags.map((tag) => titleCase(tag))),
    occasionTags: canonicalizeOccasionTags(candidate.occasionTags),
    aiSearchIntents: dedupeStrings(candidate.aiSearchIntents.map((intent) => intent.trim()).filter(Boolean)),
  };
}

export function buildCandidateKeywordCorpus(candidate: ProductSearchCandidate) {
  return normalizeSearchText(
    [
      ...
      ...candidate.styleTags,
      ...candidate.vibeTags,
      ...candidate.occasionTags,
      ...candidate.aiSearchIntents,
    ].join(' '),
  );
}

export function calculateMetadataDepthScore(candidate: ProductSearchCandidate) {
  let score = 0;
  if (candidate.title.trim().length >= 8) score += 1;
  if (candidate.description.trim().length >= 40) score += 1;
  if (candidate.styleTags.length >= 2) score += 1;
  if (candidate.occasionTags.length >= 1) score += 1;
  if (candidate.aiSearchIntents.length >= 3) score += 1;
  if (candidate.vibeTags.length >= 1) score += 1;
  if (candidate.color.trim()) score += 1;
  if (candidate.fit.trim()) score += 1;
  if (candidate.sizes.length >= 2) score += 1;
  return score;
}
...
function findEntriesInText<T extends string>(text: string, dictionary: Array<VariantEntry<T>>) {
  ...
  const tokens = new Set(tokenizeSearchText(normalizedText));
  return dictionary
    .filter((entry) =>
      entry.variants.some((variant) => {
        ...
        return normalizedVariant.includes(' ')
          ...
          : tokens.has(WORD_NORMALIZATION_MAP[normalizedVariant] ?? normalizedVariant);
      }))
